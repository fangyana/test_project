...
import com.f.pro.common.util.SpringUtil;
import com.f.pro.data.enums.DataScopeTypeEnum;
import com.f.pro.data.strategy.AbstractDataScopeHandler;
import com.f.pro.data.strategy.DataScopeContext;
import com.f.pro.data.strategy.ThisLevelChildrenDataScope;
import com.f.pro.data.strategy.ThisLevelDataScope;
import com.f.pro.security.domaim.ProSecurityUser;
import com.f.pro.security.util.SecurityUtil;
import com.f.pro.common.exception.CheckedException;
...
public class DataScopeInterceptor extends AbstractSqlParserHandler implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (CollUtil.isEmpty(deptIds)) {
            ProSecurityUser user = SecurityUtil.getUser();
            if (user == null) {
            ...
            // 生成数据访问策略
            Map<String, AbstractDataScopeHandler> strategyMap = new HashMap<>();
            strategyMap.put("2", (ThisLevelDataScope) springUtil.getObject("2"));//,new ThisLevelDataScope());
            strategyMap.put("3", (ThisLevelChildrenDataScope) springUtil.getObject("3"));//,new ThisLevelChildrenDataScope());
            dataScopeContext = new DataScopeContext(strategyMap);
            // 遍历所有角色获取各角色对应的部门id
            for (String roleId : roleIdList) {
                Entity query = Db.use(dataSource).queryOne("SELECT * FROM sys_role WHERE role_id ='" + roleId + "'");
                ...
                if (DataScopeTypeEnum.THIS_LEVEL.getType() == dsType ||
                        DataScopeTypeEnum.THIS_LEVEL_CHILDREN.getType() == dsType) {
                    List<Integer> ids = dataScopeContext.getDeptIdsForDataScope(null, dsType);
                    deptIds.addAll(ids);
                }
            }
            /*// 通过角色Id查询范围权限
