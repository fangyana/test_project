 ...
 * @Author: FangYN
 * @Date: 2020/4/14
 * @Description: 极光推送工具包 TODO 待验证 需要注册极光推送获取 secret 及 appkey
 */
@Component
public class JPushUtil {
    ...
    //这里的配置可以配置在配置文件中
    @Value("${jiguang.isProduct}") //是否是生产环境 开发环境false，生产环境true
    private static Boolean ApnsProduction;
    @Value("${jiguang.masterSecret}")
    private static String masterSecret;//极光应用的secret
    @Value("${jiguang.appKey}")
    private static String appKey;//极光应用的appkey

    private static JPushClient getConnect() {
        JPushClient jpushClient = new JPushClient(masterSecret, appKey, null, ClientConfig.getInstance());
        return jpushClient;
    }

    private static void closeClient(JPushClient jpushClient) {
        if (jpushClient != null)
            jpushClient.close();
    }
     ...
     * @return 0推送失败，1推送成功
     */
    public static int sendToAliasList(List<String> aliasList, String msg_content, String extraKey, String extrasparam, String notification_title) {
        int result = 0;
        JPushClient jPushClient = null;
        try {
            jPushClient = getConnect();
